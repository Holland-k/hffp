...
instance Show Puzzle where
    show (Puzzle word discovered guessed bad) = 
        "Word is: " ++ word ++ " disc: " ++ show discovered ++ " guessed: " ++ show guessed ++ 
            " bad: " ++ show bad
    --(intersperse ' ' $ 
        --fmap renderPuzzleChar discovered) ++ " Guessed so far: " ++ guessed

instance Eq Puzzle where 
    (==) (Puzzle a b c d) (Puzzle a' b' c' d') = 
        (a == a' && b == b' && c == c' && d == d')
...
fillInCharacter (Puzzle word filledInSoFar s b) c nb = 
    ...
    where 
        ...
        newB = if nb == Nothing then b else b++[c]
